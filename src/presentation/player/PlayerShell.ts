...
export class PlayerShell {
  ...
  public mount(): void {
    if (this.mounted || this.lifecycle.signal.aborted) return;
    ...
    buttonRow.append(backwardButton, playbackButton, forwardButton, fitButton);
    controls.append(timelineControl.create(), buttonRow);
    root.append(this.media, controls);
    document.body.replaceChildren(root);

    backwardButton.addEventListener("click", () => this.navigate(-DEFAULT_SEEK_SECONDS), { signal: this.lifecycle.signal });
    playbackButton.addEventListener("click", () => this.togglePlayback(), { signal: this.lifecycle.signal });
    forwardButton.addEventListener("click", () => this.navigate(DEFAULT_SEEK_SECONDS), { signal: this.lifecycle.signal });

    for (const eventName of ["play", "pause", "ended"] as const) {
      this.media.addEventListener(eventName, () => this.updatePlaybackButton(), { signal: this.lifecycle.signal });
    }
  ...
  private createNavigationButton(document: Document, label: string, direction: NavigationDirection): HTMLButtonElement {
  ...
  private navigate(deltaSeconds: number): void {
    try {
      if (!seekBy(this.media, deltaSeconds)) this.logger.debug("No safe media navigation target is currently available.");
    } catch (error) {
  ...
  private installStyles(document: Document): void {
    ...
    style.textContent = `
      ...
      .floatplay-playback-button { display: grid; place-items: center; flex: 0 0 28px; width: 28px; height: 28px; padding: 0; border: 0; border-radius: 999px; box-sizing: border-box; line-height: 0; color: #fff; background: rgb(0 0 0 / 68%); cursor: pointer; pointer-events: auto; transition: background-color 100ms ease; }
      .floatplay-playback-button:hover { background: rgb(0 0 0 / 58%); }
      .floatplay-playback-button:active { background: rgb(0 0 0 / 48%); }
      .floatplay-playback-button:focus-visible, .floatplay-timeline:focus-visible { outline: 2px solid #fff; outline-offset: 2px; }
      .floatplay-playback-icon, .floatplay-navigation-icon { display: block; place-self: center; margin: 0; pointer-events: none; }
      .floatplay-playback-icon { fill: currentColor; }
      .floatplay-navigation-icon { flex: none; }
      .floatplay-timeline-group { display: flex; align-items: center; gap: 10px; width: 100%; pointer-events: auto; }
      .floatplay-timeline { --floatplay-timeline-progress: 0%; flex: 1; min-width: 0; height: 20px; margin: 0; appearance: none; background: transparent; cursor: pointer; }
      .floatplay-timeline::-webkit-slider-runnable-track { height: 4px; border-radius: 999px; background: linear-gradient(to right, #fff 0 var(--floatplay-timeline-progress), rgb(255 255 255 / 35%) var(--floatplay-timeline-progress) 100%); }
      .floatplay-timeline::-webkit-slider-thumb { width: 12px; height: 12px; margin-top: -4px; appearance: none; border: 0; border-radius: 999px; background: #fff; }
      .floatplay-timeline:hover::-webkit-slider-runnable-track { background: linear-gradient(to right, #fff 0 var(--floatplay-timeline-progress), rgb(255 255 255 / 48%) var(--floatplay-timeline-progress) 100%); }
      .floatplay-timeline:disabled { cursor: default; opacity: 0.5; }
      .floatplay-time-display { flex: none; color: #fff; font: 500 12px/1.2 system-ui, sans-serif; font-variant-numeric: tabular-nums; text-shadow: 0 1px 2px rgb(0 0 0 / 75%); user-select: none; }
      @media (prefers-reduced-motion: reduce) { .floatplay-playback-button { transition: none; } }
    `;

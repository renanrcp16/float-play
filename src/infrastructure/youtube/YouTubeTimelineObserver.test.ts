...
describe("isYouTubeWatchTimelineMutation", () => {
  it("accepts authoritative progress-coordinate updates from the YouTube watch slider", () => {
    expect(
      isYouTubeWatchTimelineMutation(
        timelineMutation("aria-valuenow", true)
      )
    ).toBe(true);
    expect(
      isYouTubeWatchTimelineMutation(
        timelineMutation("aria-valuemin", true)
      )
    ).toBe(true);
    expect(
      isYouTubeWatchTimelineMutation(
        timelineMutation("aria-valuemax", true)
      )
    ).toBe(true);
  });
  ...
  it("ignores unrelated attributes and unrelated sliders", () => {
    expect(
      isYouTubeWatchTimelineMutation(
        timelineMutation("aria-valuetext", true)
      )
    ).toBe(false);
    expect(
      isYouTubeWatchTimelineMutation(
        timelineMutation("aria-valuenow", false)
      )
    ).toBe(false);
  });
});

...
const FALLBACK_SIZE: PipWindowSize = { width: 480, height: 270 };
...
export function calculateInitialPipSize(mediaWidth: number, mediaHeight: number): PipWindowSize {
  ...
  if (aspectRatio >= 1) {
    return { width: TARGET_LONG_SIDE, height: Math.max(1, Math.round(TARGET_LONG_SIDE / aspectRatio)) };
  }

  return { width: Math.max(1, Math.round(TARGET_LONG_SIDE * aspectRatio)), height: TARGET_LONG_SIDE };
}
